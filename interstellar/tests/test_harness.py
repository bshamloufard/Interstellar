...
class MaterializeTests(unittest.TestCase):
    ...
    def test_materialize_seals_config_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            auth_dir = self._auth_dir(tmp)
            version = _version([_skill("alpha", TEN_LINES)])
            dest = Path(tmp) / "grok_home"
            ...
            self.assertIn(harness._SEAL_WARNING, version["warnings"])
    ...
    def test_materialize_seal_false_opts_out(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn(harness._SEAL_WARNING, version["warnings"])

    def test_materialize_does_not_duplicate_seal_on_rematerialize(self):
        with tempfile.TemporaryDirectory() as tmp:
            auth_dir = self._auth_dir(tmp)
            version = _version([_skill("alpha", TEN_LINES)])
            dest = Path(tmp) / "grok_home"
            harness.materialize(version, dest, auth_from=auth_dir)
            harness.materialize(version, dest, auth_from=auth_dir)
            self.assertEqual(version["warnings"].count(harness._SEAL_WARNING), 1)
            text = (dest / "config.toml").read_text()
